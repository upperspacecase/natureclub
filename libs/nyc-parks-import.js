/**
 * NYC Parks event import — shared by scripts/import-nyc-parks-events.mjs
 * (manual CLI run) and /api/cron/import-events (weekly cron).
 *
 * Source: NYC Open Data "NYC Parks Public Events – Upcoming 14 Days"
 * (dataset w3wp-dpdi), refreshed daily by DPR.
 *
 * Framework-free on purpose: no path aliases and no model imports, so the
 * plain-node script can import it relatively. Callers hand in a native
 * MongoDB `db` handle.
 */
...
// The feed's starttime/endtime carry the correct TIME OF DAY but their date
// portion is stamped with the feed refresh date — real dates live in
// startdate/enddate. NYC is UTC-4 (EDT) for the season this feed covers;
// worst case in winter an event displays an hour off until refresh.
...
export async function fetchParksEventDocs() {
    ...
    for (const r of rows) {
        ...
        candidates.push({
            ...
            doc: {
                ...
                currency: "USD",
                coverPhotoUrl:
                    DEFAULT_COVERS[activityType] || DEFAULT_COVERS.other,
    ...
    return { totalRows: rows.length, aligned: candidates.length, finalDocs };
}

/**
 * Upsert the selected docs into the given native MongoDB db handle under the
 * "natureclub" curator user. Returns counts.
 */
export async function importParksEvents(db) {
    const { totalRows, aligned, finalDocs } = await fetchParksEventDocs();
    ...
    return { totalRows, aligned, selected: finalDocs.length, created, updated };
}
